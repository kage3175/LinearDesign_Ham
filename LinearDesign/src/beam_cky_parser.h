...
public:
    ...
    //==========================================================================
    int penalty_mode = PENALTY_OFF;
    std::vector<std::vector<std::pair<IndexType, IndexType>>> penalty_vec;
    //==========================================================================
...
private:
    ...
    //==========================================================================
    void update_if_better(State_t &state, const ScoreType newscore, const double cai_score, IndexType i) {
        if (state.score + state.cai_score < newscore + cai_score) {
            if (is_penalty(i)) {
                state.score = INT_MIN;
            } else {
    ...
    void update_if_better(State_t &state, const ScoreType newscore, const double cai_score, IndexType i, IndexType j) {
        if (state.score + state.cai_score < newscore + cai_score) {
            if (is_penalty(i, j)) {
                state.score = INT_MIN;
            } else {
    ...
    void update_if_better(State_t &state, const ScoreType newscore, const double cai_score, const NodeType pre_node, const double pre_left_cai, IndexType i) {
        if (state.score + state.cai_score < newscore + cai_score) {
            if (is_penalty(i)) {
                state.score = INT_MIN;
            } else {
    ...
    void update_if_better(State_t &state, const ScoreType newscore, const double cai_score, const NodeType pre_node, const double pre_left_cai, IndexType i, IndexType j) {
        if (state.score + state.cai_score < newscore + cai_score) {
            if (is_penalty(i, j)) {
                state.score = INT_MIN;
            } else {
    ...
    bool is_penalty(IndexType i) {
        if(penalty_mode == PENALTY_ON){
            // penalty_vec -> vector<vector<pair<IndexType, IndexType>>>
            for(const auto& vec1: penalty_vec) {
                for(const auto& pair: vec1){
                    if(pair.first <= i && i <= pair.second) return true;
                }
            }
        }
        return false;
    ...
    bool is_penalty(IndexType i, IndexType j) {
        if (penalty_mode == PENALTY_ON) {
            for(const auto& group: penalty_vec) {
                int i_pair_idx = -1, j_pair_idx = -1;

                for(size_t a = 0; a < group.size(); ++a){
                    if(group[a].first <= i && group[a].second >= i){
                        i_pair_idx = a;
                    }
                    if(group[a].first <= j && group[a].second >= j){
                        j_pair_idx = a;
                    }
                }
                if(i_pair_idx != -1 && j_pair_idx != -1 && i_pair_idx != j_pair_idx){
                    return true;
                }
            }
        }
