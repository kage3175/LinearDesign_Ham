...
template <typename IndexType,
          typename NodeType = pair<IndexType, NumType>,
          typename DFAType = DFA<IndexType>>
DFAType get_dfa(const string& filename, string& utr_seq) {
    DFAType dfa = DFAType();
    ifstream file(filename);
    string line;
    string full_seq;
    string protein_seq;
    pair<IndexType, NumType> current_node;
    ...
    while (getline(file, line)) {
        if (line.rfind("node:", 0) == 0) {
            int pos, num;
            sscanf(line.c_str(), "node:\t(%d, %d)", &pos, &num);
            current_node = make_pair(pos, num);
            dfa.add_node(current_node);
        } else if (line.rfind("R_", 0) == 0 || line.rfind("L_", 0) == 0) {
            char direction;
            int idx;
            char nuc_char;
            double weight;
            int p2, num2;
            sscanf(line.c_str(), "%c_%d:\t%c;%lf;(%d, %d)", &direction, &idx, &nuc_char, &weight, &p2, &num2);
            int nuc = GET_ACGU_NUC(nuc_char);
            auto neighbor_node = make_pair(p2, num2);
            if (direction == 'R')
                dfa.add_edge(current_node, neighbor_node, nuc, weight);
        }
    }
    file.close();
    return dfa;
