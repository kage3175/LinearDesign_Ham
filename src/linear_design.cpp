...
void show_usage() {
    cerr << "echo SEQUENCE | ./lineardesign -l [LAMBDA] -v -c [codon table] -u3 [3' UTR] -p [penalty string]" << endl;
    cerr << "OR" << endl;
    cerr << "cat SEQ_FILE_OR_FASTA_FILE | ./lineardesign -l [LAMBDA] -v -c [codon table] -u3 [3' UTR] -p [penalty string]" << endl;
}
...
BeamCKYParser<ScoreType, IndexType>& parse_penalty_options(BeamCKYParser<ScoreType, IndexType>& parser, string penalty_str) {
    ...
    while(std::getline(stream, group, '/')){
        vector<pair<IndexType, IndexType>> pairs;
        stringstream group_ss(group);
        string pair_str;

        while(std::getline(group_ss, pair_str, ',')){
            stringstream pair_ss(pair_str);
            string num;
            int a, b;
        ...
        parser.penalty_vec.emplace_back(pairs);
    }
